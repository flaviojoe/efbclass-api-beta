...
from efbclass.storage.cache import cache_it
from efbclass.storage.exceptions import SpaceNotEnoughtException, UnknownIdException

'''
	baseado em: https://github.com/suquant/django-vimeo/blob/master/django_vimeo/storage.py
'''
...
class VimeoFileStorage(Storage):
	def __init__(self, client_id=None, client_secret=None, access_token=None):
		...
		self.oembed_url = getattr(settings, 'VIMEO_OEMBED_URL', 'https://vimeo.com/api/oembed.json')
		self.video_url_pattern = getattr(settings, 'VIMEO_VIDEO_URL_PATTERN', 'https://vimeo.com/{}')

	@cached_property
	def client(self):
		return vimeo.VimeoClient(token=self.access_token, key=self.client_id, secret=self.client_secret)
